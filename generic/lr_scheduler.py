...
class RampUpCosineLR(_LRScheduler):
    """ LRScheduler ramps up the learning rate from `annealing` to the
    `base_lr` over `ramp_up` epochs and then apply cosine annealing between
    `annealing` and `base_lr` with period `period`.
    """
    def __init__(self, optimizer, ramp_up, period, annealing):
        self._period = period
        self._ramp_up = ramp_up
    ...
    def get_lr(self):
        if self.last_epoch < self._ramp_up:
            alpha = self.last_epoch / self._ramp_up
            return [
                (1 - alpha) * self._annealing + alpha * base_lr
                for base_lr in self.base_lrs
            ]
        else:
            return [
                self._annealing + (base_lr - self._annealing) *
                (1 + math.cos(
